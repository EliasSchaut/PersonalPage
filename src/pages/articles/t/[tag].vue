<template>
  <LayoutHeader
    :title="`${$t('articles.title')} (${$route.params.tag})`"
  ...
  >
    ...
    <nav>
      <ContentList :path="$i18n.locale + '/articles'" v-slot="{ list }">
        <ul
          class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-second-200 pt-10 dark:border-second-500 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3"
        >
          <li
            class="text-second-700 dark:text-second-300"
            v-if="
              !list.some((article: any) =>
                article.tags.includes($route.params.tag),
              )
            "
          >
